...
import { DonHangInListDto } from '@/proxy/viet-life/business/don-hangs-list/don-hangs';
...
export class DonHangComponent implements OnInit, OnDestroy {
  ...
  deleteItems() {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => this.deleteItemsConfirmed(this.selectedItems.map(x => x.id))
    });
  }
  ...
  private toggleBlockUI(enabled: boolean) {
    if(enabled){
      ...
      }, 1000); // delay nhẹ để tránh nháy UI
    }
  }
}
